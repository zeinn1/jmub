...
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**سورس بوت كرستين  يعمل بنجاح**"
...
def jmthonalive_text():
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    jmthon_caption = "**سورس بوت كرستين  يعمل بنجاح**\n"
    jmthon_caption += f"**{EMOJI} اصدار التيليثون :** `{version.__version__}\n`"
    jmthon_caption += f"**{EMOJI} اصدار بوت كرستين  :** `{jmthonversion}`\n"
...
ROZ_PIC = "https://telegra.ph//file/7fe89d5c10daf7a62fddd.jpg"
RAZAN = Config.TG_BOT_USERNAME
ROZ_T = (
    f"**⌯︙بوت بوت كرستين يعمل بنجاح 🤍،**\n"
    f"**   - اصدار التليثون :** `1.23.0\n`"
    f"**   - اصدار بوت كرستين  :** `4.0.0`\n"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("السورس") and event.query.user_id == jmub.uid:
            buttons = [
                [
                    Button.url("قنـاة السـورس ⚙️", "https://t.me/S_EG_P"),
                    Button.url("المطـور 👨🏼‍💻", "https://t.me/devpokemon"),
                ]
            ]
