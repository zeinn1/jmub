...
async def upstream(event):
    ...
    try:
        txt = "فشل في التحديث لسورس كرستين  " + "**• حدث خطأ ما :**\n"
    ...
    if changelog == "" and not force_update:
        await event.edit(
            "\n**• سورس كرستين  محدث الى أخر اصدار**"
    ...
    if conf == "" and not force_update:
        ...
        return await event.respond(
            f"**• ارسل** `{cmdhd}تحديث التنصيب` لتحديث سورس كرستين "
        )

    if force_update:
        await event.edit("**• جار التحديث الاجباري الى اخر اصدار انتظر قليلا**")
    if conf == "الان":
        await event.edit("**• جار تحديث سورس كرستين  أنتظر قليلا**")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/zeinn1/zein"
    os.chdir("/app")
    try:
